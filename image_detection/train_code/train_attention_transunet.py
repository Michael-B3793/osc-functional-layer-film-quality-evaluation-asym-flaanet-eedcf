...
OUT_DIR = "../Comparison_And_Ablation_Runs/03_Attention_TransUNet"
TARGET_SIZE = 576
EPOCHS = 300
BATCH_SIZE = 8
NUM_WORKERS = 8
...
class SingleOutputLoss(nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def forward(self, logits, gt):
        loss_bce = self.bce(logits, gt)
        loss_dice = self.dice(logits, gt)
...
def train_transunet_baseline():
    set_seed(SEED)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if device.type != 'cuda':
        print("\n Warning: PyTorch did not detect 5090 GPU! Running at slow speed!")
    else:
        print(f"\n Full power: Detected {torch.cuda.get_device_name(0)}, starting TransUNet baseline training mode...\n")
    ...
    print("Building TransUNet (Mit-B3 + scSE Attention)...")
    model = smp.Unet(encoder_name="mit_b3", encoder_weights="imagenet", in_channels=3, classes=2, decoder_attention_type="scse").to(device)
    opt = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    sch = torch.optim.lr_scheduler.ReduceLROnPlateau(opt, mode="min", factor=0.5, patience=15)
    loss_fn = SingleOutputLoss()
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        log_str = (f"[TransUNet - Ep {ep:03d}/{EPOCHS}] LR={curr_lr:.2e} | "
                  f"Tr[Loss:{tr_loss:.3f}, IoU:{tr_iou:.3f}] | "
                  f"Val[Loss:{va_loss:.3f}, IoU:{va_iou:.3f}] | "
    ...
    print(f"\nTraining completed! TransUNet total time: {time.strftime('%H:%M:%S', time.gmtime(total_time))}")

if __name__ == "__main__":
    train_transunet_baseline()