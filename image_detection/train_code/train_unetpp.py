...
OUT_DIR = "../Comparison_And_Ablation_Runs/02_Structure_UNetPP"
...
def read_height_proxy(img_path):
    ...
    if img is None: raise RuntimeError(f"Failed: {img_path}")
...
def train_unetpp():
    ...
    if device.type != 'cuda':
        ...
        print(f"\n GPU detected: {torch.cuda.get_device_name(0)}, starting U-Net++ training...\n")
        print(f"\n Training images directory: {TRAIN_IMG_DIR}")
        print(f" Training annotations directory: {TRAIN_JSON_DIR}")
        print(f" Validation images directory: {VAL_IMG_DIR}")
        print(f" Validation annotations directory: {VAL_JSON_DIR}")
        print(f" Output directory: {OUT_DIR}")
        print("\n Checking if directories exist:")
        print(f" Training images directory exists: {os.path.exists(TRAIN_IMG_DIR)}")
        print(f" Training annotations directory exists: {os.path.exists(TRAIN_JSON_DIR)}")
        print(f" Validation images directory exists: {os.path.exists(VAL_IMG_DIR)}")
        print(f" Validation annotations directory exists: {os.path.exists(VAL_JSON_DIR)}")

        if os.path.exists(TRAIN_IMG_DIR):
            train_files = [f for f in os.listdir(TRAIN_IMG_DIR) if f.lower().endswith((".tif", ".tiff", ".png", ".jpg"))]
            print(f"\n Training set file count: {len(train_files)}")
            if train_files:
                print(f" Example files: {train_files[:3]}...")

                if os.path.exists(VAL_IMG_DIR):
                    val_files = [f for f in os.listdir(VAL_IMG_DIR) if f.lower().endswith((".tif", ".tiff", ".png", ".jpg"))]
                    print(f" Validation set file count: {len(val_files)}")
                    if val_files:
                        print(f" Example files: {val_files[:3]}...")

                        os.makedirs(OUT_DIR, exist_ok=True)
                        
                        print("\n Creating datasets...")
                        ds_tr = AFMBWDataset(TRAIN_IMG_DIR, TRAIN_JSON_DIR, TARGET_SIZE, train=True)
                        print(f" Training set size: {len(ds_tr)}")
                        ds_va = AFMBWDataset(VAL_IMG_DIR, VAL_JSON_DIR, TARGET_SIZE, train=False)
                        print(f" Validation set size: {len(ds_va)}")
                        ...
                        print(f" Training data loader batches: {len(dl_tr)}")
                        print(f" Validation data loader batches: {len(dl_va)}")

                        model = smp.UnetPlusPlus(encoder_name="resnet34", encoder_weights="imagenet", in_channels=3, classes=2).to(device)
                        ...
                        print(f"\n U-Net++ training completed! Total time: {time.strftime('%H:%M:%S', time.gmtime(total_time))}")
