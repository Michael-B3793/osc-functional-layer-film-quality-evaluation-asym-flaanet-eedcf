...
OUT_DIR = "../Comparison_And_Ablation_Runs/06_Ablation_NoASPP"
TARGET_SIZE = 576
EPOCHS = 300
BATCH_SIZE = 8
NUM_WORKERS = 8
...
def train_ablation_no_aspp():
    set_seed(SEED)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    if device.type != 'cuda':
        print("\n Warning: No 5090 GPU detected! Running at slow speed!\n")
    else:
        print(f"\n Detected heavy firepower: {torch.cuda.get_device_name(0)}! Starting [No ASPP Ablation Experiment]...\n")
    ...
    for ep in range(1, EPOCHS + 1):
        ...
        log_str = (f"[NoASPP - Ep {ep:03d}/{EPOCHS}] LR={curr_lr:.2e} | "
    ...
    print(f"\nAblation experiment (No ASPP) completed! Total time: {time.strftime('%H:%M:%S', time.gmtime(total_time))}")

if __name__ == "__main__":
    train_ablation_no_aspp()