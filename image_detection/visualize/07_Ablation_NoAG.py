...
class Config:
    ...
    MODEL_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "Comparison_And_Ablation_Runs/07_Ablation_NoAG/best.pth")
...
class Model_NoAG(nn.Module):
    ...
    def forward(self, x):
        x0 = self.stem(x); x1 = self.d1(x0); x2 = self.d2(x1); m = self.mid(x2)
