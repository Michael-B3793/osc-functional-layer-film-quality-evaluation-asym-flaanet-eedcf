...
class Config:
    ...
    MODEL_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "Comparison_And_Ablation_Runs/03_Attention_TransUNet/best.pth")
...
if __name__ == "__main__":
    ...
    model = smp.Unet(encoder_name="mit_b3", encoder_weights=None, in_channels=3, classes=2, decoder_attention_type="scse")
    ...
    if not image_files:
        ...
        for img_name in tqdm(image_files, desc="Processing Images"):
            ...
            with torch.no_grad():
                ...
                mask_w_pred = (prob[0] > Config.MASK_THRESH).astype(np.uint8)
                mask_b_pred = (prob[1] > Config.MASK_THRESH).astype(np.uint8)
