...
class Config:
    ...
    TEST_JSON_DIR = os.path.join(os.path.dirname(SCRIPT_DIR), "dataset_400/text_jsons")
    WHITE_EXPERT_MODEL_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "Comparison_And_Ablation_Runs/04_Strategy_Dual_WhiteExpert/best.pth")
    BLACK_EXPERT_MODEL_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "Comparison_And_Ablation_Runs/05_Strategy_Dual_BlackExpert/best.pth")
    CURRENT_FILE_NAME = os.path.splitext(os.path.basename(__file__))[0]
    OUTPUT_IMG_DIR = os.path.join(SCRIPT_DIR, CURRENT_FILE_NAME)
    OUTPUT_CSV_PATH = os.path.join(SCRIPT_DIR, f"{CURRENT_FILE_NAME}/Detection_Metrics.csv")
    ...
    def __init__(self, in_ch, out_ch, rates=(1, 6, 12, 18)):
        ...
        for r in rates[1:]:
            self.branches.append(
                nn.Sequential(nn.Conv2d(in_ch, out_ch, 3, padding=r, dilation=r, bias=False), nn.BatchNorm2d(out_ch),
                nn.SiLU(inplace=True))
            )
        self.global_pool = nn.Sequential(nn.AdaptiveAvgPool2d(1), nn.Conv2d(in_ch, out_ch, 1, bias=False),
        nn.BatchNorm2d(out_ch), nn.SiLU(inplace=True))
        self.proj = nn.Sequential(nn.Conv2d(out_ch * 5, out_ch, 1, bias=False), nn.BatchNorm2d(out_ch),
        nn.SiLU(inplace=True))
...
def process_contours_to_metrics(contours, d_type, img_name, source_name):
    ...
    for cnt in contours:
        ...
        metrics.append({
            "Image": img_name, "Source": source_name, "Type": d_type, "ID": d_id,
            "CX": cx, "CY": cy, "X_min": x, "Y_min": y, "W": w, "H": h, "Area": round(area, 2)
        })
        d_id += 1
    return metrics
...
if __name__ == "__main__":
    ...
    print("Loading area segmentation model...")
    white_expert_model = smp.Unet(encoder_name="mit_b3", encoder_weights=None, in_channels=3, classes=1, decoder_attention_type="scse")
    white_expert_model.load_state_dict(torch.load(Config.WHITE_EXPERT_MODEL_PATH, map_location=Config.DEVICE))
    white_expert_model.to(Config.DEVICE)
    white_expert_model.eval()
    black_expert_model = smp.Unet(encoder_name="mit_b3", encoder_weights=None, in_channels=3, classes=1, decoder_attention_type="scse")
    black_expert_model.load_state_dict(torch.load(Config.BLACK_EXPERT_MODEL_PATH, map_location=Config.DEVICE))
    black_expert_model.to(Config.DEVICE)
    black_expert_model.eval()
    target_image = "4.tiff"
    ...
    if not image_files:
        ...
        for img_name in tqdm(image_files, desc="Processing Images"):
            ...
            z01_white = cv2.resize(robust_norm01(z), (Config.TARGET_SIZE, Config.TARGET_SIZE))
            z01_c_white = clahe01(detrend_by_gaussian(z01_white))
            ch2_white, ch3_white = local_rms(z01_c_white), grad_mag(z01_c_white)
            x_white = torch.from_numpy(np.stack([z01_c_white, ch2_white, ch3_white])).unsqueeze(0).to(Config.DEVICE)
            z01_black = cv2.resize(robust_norm01(z), (Config.TARGET_SIZE, Config.TARGET_SIZE))
            z01_inverted = 1.0 - detrend_by_gaussian(z01_black)
            z01_c_black = clahe01(z01_inverted)
            ch2_black, ch3_black = local_rms(z01_c_black), grad_mag(z01_c_black)
            x_black = torch.from_numpy(np.stack([z01_c_black, ch2_black, ch3_black])).unsqueeze(0).to(Config.DEVICE)
            orig_color = cv2.imread(img_path, cv2.IMREAD_COLOR)
            canvas_gt = cv2.resize(orig_color, (Config.TARGET_SIZE, Config.TARGET_SIZE))
            canvas_pred = canvas_gt.copy()
            c_w_gt, c_b_gt = [], []
            ...
            with torch.no_grad():
                white_prob = torch.sigmoid(white_expert_model(x_white)[0][0]).cpu().numpy()
                black_prob = torch.sigmoid(black_expert_model(x_black)[0][0]).cpu().numpy()
                mask_w_pred = (white_prob > Config.MASK_THRESH).astype(np.uint8)
                mask_b_pred = (black_prob > Config.MASK_THRESH).astype(np.uint8)
        ...
        if all_metrics:
            ...
            print(f"\nEvaluation completed! Recorded {len(df)} area data entries.")
