...
def extract_and_calculate_eedcf(prob_w, prob_b, z_raw):
    print("\n" + "="*50)
    print("Algorithm 3: EEDCF (Energy-Entropy Defect Characterization Function) Calculation Report")
    print("="*50)
    ...
    print(f" Extracting background features: Global brightness={mu_global:.4f} -> Cosine health correction Gamma_bg = {Gamma_bg:.4f}")
    
    mask_w, mask_b = (prob_w > Config.MASK_THRESH).astype(np.uint8), (prob_b > Config.MASK_THRESH).astype(np.uint8)
    ...
    N, A_def, sum_Ek = 0, 0.0, 0.0
    print("\nCalculating microscopic morphology energy (Ek) for each defect...")
    for idx, cnt in enumerate(c_w + c_b):
        ...
        print(f"  -> Defect {idx+1}: Shape={ratio:.2f}, Brightness deviation={color_penalty:.2f} => Exponential activation energy Ek = {Ek:.4f}")
    
    if N == 0:
        print(f"\nFinal EEDCF score: {(Gamma_bg ** Config.W6):.4f} (no defects state)")
        return
    
    A_avg = A_def / N
    p_area, p_avg, p_count = min(A_def/Config.TOL_AREA, 1.0), min(A_avg/Config.TOL_AVG_AREA, 1.0), min(N/Config.TOL_COUNT, 1.0)
    
    print("\nCalculating macro defect entropy (Minkowski distance fusion)...")
    term1, term2, term3 = (Config.W1 * p_area)**2, (Config.W2 * p_avg)**2, (Config.W4 * p_count)**2
    H_defect = math.sqrt(term1 + term2 + term3)
    print(f"  -> A_def penalty: {math.sqrt(term1):.4f} | A_avg penalty: {math.sqrt(term2):.4f} | N penalty: {math.sqrt(term3):.4f}")
    print(f"  Fusion macro entropy H_defect = {H_defect:.4f}")
    
    mean_Ek = sum_Ek / N
    EEDCF = (Gamma_bg ** Config.W6) * (mean_Ek / (1.0 + H_defect))
    print(f"\nFinal EEDCF energy characterization operator = (Gamma_bg^W6) * [mean_Ek / (1 + H_defect)]")
    print(f"                                         = ({Gamma_bg:.4f}^{Config.W6}) * [{mean_Ek:.4f} / (1 + {H_defect:.4f})]")
    print(f"                                         = {EEDCF:.6f} [0,1 range, closer to 1 is better]")
    print("="*50)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        ...
        extract_and_calculate_eedcf(prob[0], prob[1], z_raw)
