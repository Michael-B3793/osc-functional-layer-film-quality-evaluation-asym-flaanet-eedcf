...
def run_for_target(target_col: str, output_dir_name: str, axis_label: str):
    plt.rcParams.update(
        {
            "font.family": "Arial",
    ...
    safe_target = re.sub(r"[^a-zA-Z0-9]+", "_", target_col.lower()).strip("_")
    root = Path(__file__).resolve().parents[1]
    data_path = root / "result" / "data.csv"
    out_dir = root / "result" / output_dir_name
    ...
    rank_df = pd.DataFrame({target_col: df[target_col]})
    rank_df[f"{target_col}_rank"] = rank_df[target_col].rank(method="average")
    for m in metrics:
    ...
    ax.set_xlabel(f"{axis_label} rank")
    ...
    bins = pd.qcut(df[target_col], 4, labels=["Q1_low", "Q2", "Q3", "Q4_high"])
    binned = (
        df.assign(Target_bin=bins)
        .groupby("Target_bin", as_index=False)[[target_col, "EEDCF", "ASDI", "MMCSO_dB"]]
        .mean()
    )
    ...
    ax1.set_box_aspect(1)
    axes_map = {"EEDCF": ax1, "ASDI": ax2, "MMCSO_dB": ax3}
    x_bins = np.arange(len(binned["Target_bin"]))
    legend_handles = []
    legend_labels = []
    for m in metrics:
        ax = axes_map[m]
    ...
    ax1.set_xticklabels(binned["Target_bin"])
    ax1.set_xlabel(f"{axis_label} quartile")
    ax1.legend(
        legend_handles,
        legend_labels,
        loc="upper left",
